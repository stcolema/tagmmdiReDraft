...
{
public:
  std::vector<mixture> ver;
  VecWrapper(uword L, uvec K, mat labels, cube X)
  {
    ver.reserve(L);
    for(arma::uword l = 0; l < L; ++l ){
      ver.push_back(mvnMixture(K(l), labels.ncol(l), X.slice(l)));
    } 
  }
};
...
public:
  ...
  // Cube of datasets, cluster members
  arma::cube X, members;
  ...
  mdiModel(
    ...
    one_to_K = linspace(0, K_max - 1, K_max);
    one_to_L = linspace(0, L - 1, L);
    ...
    ones_mat.set_size(LC2, n_combinations);
    ones_mat.ones;
    ...
    for(arma::uword l = 0; l < L; l++) {
      X.slice(l) = _X.slice(l);
      N_check(l) = _X.slice(l).n_rows;
  ...
  double calcWeightRate(uword lstar, uword kstar) {
    ...
    arma::mat relevant_phis_t = relevant_phis.t();
    phi_prod_mat = relevant_phis_t.each_col() % phis;
  ...
  double calcPhiRate(uword lstar, uword mstar) {
    ...
    weight_products.ones;
    
    phi_products.set_size(n_used);
    phi_products.ones;
    ...
    for(uword l = 0; l < L; l++) {
      if(l != lstar){
        weight_products *= w.col(l).rows(relevant_combinations.col(l));
      }
    }
  ...
  void updateWeights() {
    ...
    for(arma::uword l = 0; l < L; l++) {
      for(arma::uword k = 0; k < K; k++) {
        shape = 1 + accu(members.slice(l).col(k));
        rate = calcWeightRate(l, k);
        gamma(k, l) = randg(distr_param(shape, rate));
      }
    }
  }
...
public:
  ...
  virtual void updateAllocation(arma::vec weights) {
...
public:
  ...
  virtual void sampleMuPrior() {
    for(arma::uword k = 0; k < K; k++){
      mu.col(k) = arma::mvnrnd(xi, (1.0/lambda) * cov.slice(k), 1);
    }
  }
  ...
  virtual void calcBIC(){
    ...
    BIC = 2 * model_likelihood - n_param_batch * std::log(N);
  ...
  void sampleParameters() {
    ...
    for (arma::uword k = 0; k < K; k++) {
      ...
      if(n_k > 0){
        ...
        cov.slice(k) = arma::iwishrnd(scale, nu);
        mu.col(k) = arma::mvnrnd(xi, (1.0 / (double) lambda) * cov.slice(k), 1);
